    ...
    static func recommendations(
        ...
        return RecipeCatalog.recipes
            ...
            .map { recipe in
                ...
                let reason = if ratings[recipe.id] == 2 { "你上次觉得很好吃" }
                    else if recentPenalty > 0 { "最近吃过，已降低排序" }
                    else if missing.isEmpty { "现有食材完全匹配" }
                    else { "只缺 (missing.count) 种食材" }
                return Recommendation(recipe: recipe, available: available, missing: missing, score: coverage - Double(missing.count) * 0.05 + ratingBonus - recentPenalty, reason: reason)
            }
            .sorted { lhs, rhs in
